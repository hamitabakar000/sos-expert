...
import { AlertTriangle, CheckCircle2, ClipboardCheck, FileText, ShieldCheck, XCircle } from "lucide-react";
...
export function ExpertValidationBoard({ cases, experts }: ExpertValidationBoardProps) {
  ...
  const canDecide = checkedCount === selectedCase.requiredChecks.length && note.trim().length >= 30;
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          ...
          <article className="rounded-lg border border-border bg-white p-5 shadow-card">
            ...
            <p className="mt-2 text-sm text-slate-600">
              La decision reste bloquee tant que tous les controles ne sont pas coches et justifies.
            </p>
            <div className="mt-4 space-y-3">
        ...
        <article className="rounded-lg border border-border bg-white p-5 shadow-card">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-bold text-slate-700">
              {checkedCount}/{selectedCase.requiredChecks.length} controles
            </span>
          </div>
